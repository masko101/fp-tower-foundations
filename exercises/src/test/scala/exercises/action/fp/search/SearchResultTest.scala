...
class SearchResultTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("apply") {
    ...
    val flight3 = Flight("1", "BA", parisOrly, londonGatwick, now, Duration.ofMinutes(100), 1, 234.0, "")
    val flight4 = Flight("4", "LH", parisOrly, londonGatwick, now, Duration.ofMinutes(210), 2, 55.5, "")
    val result  = SearchResult(List(flight1, flight2, flight3, flight4))
    assert(result.flights == List(flight1, flight2, flight4))
  }

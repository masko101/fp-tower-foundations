...
//  }
  ...
  test("attempt success") {
    ...
    val action = IO(counter += 1).attempt(ExecutionContext.global)
    assert(counter == 0) // nothing happened before unsafeRun
  ...
  test("attempt failure") {
    ...
    val action    = IO { counter += 1; throw exception }.attempt(ExecutionContext.global)
    assert(counter == 0) // nothing happened before unsafeRun
  ...
  test("handleErrorWith success") {
    ...
    val action = first.handleErrorWith(_ => second)(global)
    assert(counter == 0) // nothing happened before unsafeRun
  ...
  test("handleErrorWith failure") {
    ...
    val second = IO(counter *= 2)
    val action = first.handleErrorWith(_ => second)(ExecutionContext.global)
    assert(counter == 0) // nothing happened before unsafeRun

...
class UserCreationExercisesTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("parseYN") {
    assert(parseYN("N") == false)
    assert(parseYN("Y") == true)
    assert(Try(parseYN("Nooooo")).isFailure)
  }
  ...
  test("readDateOfBirth example success") {
    forAll { (date: LocalDate) =>
      val console = Console.mock(ListBuffer(formatDOB(date)), ListBuffer())
  ...
  implicit val genValidMaxAttempt   = Gen.choose(1, 20)
  implicit val genInvalitMaxAttempt = Gen.negNum[Int]
  implicit val genValidYN           = Gen.oneOf("Y", "N")
  implicit val genInvalidYN         = Gen.asciiPrintableStr.suchThat(i => i != "Y" && i != "N")

  test("readSubscribeToMailingListRetry negative maxAttempt") {
    forAll(genValidYN, genInvalitMaxAttempt) { (yen, maxAttempt) =>
      val console = Console.mock(ListBuffer(yen), ListBuffer.empty[String])
      val result  = Try(readSubscribeToMailingListRetry(console, maxAttempt))
      assert(result.isFailure)
    }
  }

  test("readSubscribeToMailingListRetry examples") {
    forAll(genValidYN, Gen.choose(0, 20).flatMap(Gen.listOfN(_, genInvalidYN)), genValidMaxAttempt) {
      (yen: String, invalidYen: List[String], maxAttempt: Int) =>
        val outputs = ListBuffer.empty[String]
        val console = Console.mock(ListBuffer(invalidYen: _*) :+ yen, outputs)
        val result  = Try(readSubscribeToMailingListRetry(console, maxAttempt))

        if (invalidYen.size < maxAttempt) {
          assert(result == Success(UserCreationExercises.parseYN(yen)))
          assert(
            outputs.toList == invalidYen.flatMap(_ =>
              List("Would you like to subscribe to our mailing list? [Y/N]",

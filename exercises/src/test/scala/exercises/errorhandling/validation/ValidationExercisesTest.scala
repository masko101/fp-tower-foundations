...
class ValidationExercisesTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("validateUser example") {
    ...
    assert(
      validateUser("b!", "UK") == NEL(FieldError(FieldIds.username, NEL(TooSmall(2), InvalidCharacters(NEL('!')))),
                                      FieldError(FieldIds.countryOfResidence, NEL(InvalidFormat("UK")))).invalid
    )
  }

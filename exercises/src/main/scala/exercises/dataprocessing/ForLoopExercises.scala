...
//    total
    ...
    foldLeft(numbers, 0)((tot, num) => tot + num)
  }
...
//      s += 1
//    s
    foldLeft(items, 0)((size, num) => size + 1)
  }
...
//    for (num <- numbers)
//      min = min.fold(Some(num))(c => Some(Math.min(c, num)))
//    min
    foldLeft(numbers, Option.empty[Int]) {
      case (Some(min), num) => Some(Math.min(min, num))
  ...
  def wordCount(words: List[String]): Map[String, Int] = {
    ...
    for {
      word <- words
    } map.updateWith(word) { case Some(count) => Some(count + 1); case None => Some(1) }
    Map.from(map)
     */
    foldLeft(words, Map.empty[String, Int])((m, word) =>
      m.updatedWith(word) { case Some(count) => Some(count + 1); case None => Some(1) }
    )
  }

  def foldLeft[A, B](items: List[A], init: B)(f: (B, A) => B): B = {
    var ret = init
    for (item <- items) ret = f(ret, item)
    ret
  }
  ...
  def pattern[A, B](items: List[A], init: B)(f: (B, A) => B): B = {
    var ret = init
    for (item <- items) ret = f(ret, item)
    ret
  }

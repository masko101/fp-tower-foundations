...
object OptionExercises {
  ...
  def getAccountId(role: Role): Option[AccountId] = role match {
  ...
  def getAccountIds(users: List[User]): List[AccountId] =
    users.flatMap(u => getAccountId(u.role)).distinct
...
//  {
//    val e = users.flatMap(_.email)
//    if (e.size == users.size)
//      Some(e)
//    else
//      None
//  }
  ...
  def sequence[A](options: List[Option[A]]): Option[List[A]] = {
    //Same implementation as IO.sequence
    options
      .foldLeft(Option(List[A]())) {
        case (state, option) => option.zip(state).map { case (value, tail) => value :: tail }
      }
      .map(_.reverse)
  }
  //    options.foldLeft(Option(List[A]())) {
//      case (None, _)                 => None
//      case (_, None)                 => None
